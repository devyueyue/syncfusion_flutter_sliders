...
class SfOverlayShape {
  ...
  void paint(PaintingContext context, Offset center,
      ...
      required SfThumb? thumb}) {
    final double radius = getPreferredSize(themeData).width / 2;
    final Tween<double> tween = Tween<double>(begin: 0.0, end: 0.0);

    if (paint == null) {
      paint = Paint();
      paint.color = Colors.transparent;
    }
    context.canvas.drawCircle(Offset.zero, tween.evaluate(animation), paint);
  }
}
  ...
  Path _updateRectangularTooltipWidth(
    ...
    if (isVertical) {
      ...
      return _getRectangularPath(tooltipStartY, rightLineWidth,
          halfTooltipTriangleWidth, dy, tooltipHeight - 10, leftLineWidth,
          isVertical: isVertical);
    }
  }
